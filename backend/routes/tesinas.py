...
def listar_tesinas():
    ...
    try:
        ...
        with get_db() as conn:
            ...
            # admin: sin restricciones extra

            if search:
                conditions.append(
                    "(NORMALIZE(t.titulo) LIKE NORMALIZE(?) OR NORMALIZE(u_alumno.nombre) LIKE NORMALIZE(?) OR NORMALIZE(u_tutor.nombre) LIKE NORMALIZE(?))"
                )
    ...
    try:
        ...
        with get_db() as conn:
            ...
            cursor.execute("""
                SELECT COUNT(*)
                FROM versiones_tesinas
                WHERE tesina_id = ? AND estado_tutor = 'aprobada'
            """, (tesina_id,))
            ...
            # Primero desmarcar versiones anteriores, luego insertar la nueva
            cursor.execute("""
                UPDATE versiones_tesinas
                SET is_current = 0
                WHERE tesina_id = ?
...
def eliminar_tesina(tesina_id):
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute("SELECT nombre_archivo FROM tesinas WHERE id = ?", (tesina_id,))
            tesina = cursor.fetchone()
            if not tesina:
                return jsonify({"error": "Tesina no encontrada"}), 404

            cursor.execute("""
                SELECT nombre_archivo FROM versiones_tesinas WHERE tesina_id = ?
            """, (tesina_id,))
            versiones = cursor.fetchall()

            # Recopilar archivos únicos a eliminar
            archivos_a_eliminar = {tesina['nombre_archivo']} | {v['nombre_archivo'] for v in versiones}
            ...
            for nombre_archivo in archivos_a_eliminar:
                ...
                if os.path.exists(filepath):
                    try:
                        ...
                        logger.warning("No se pudo eliminar el archivo: %s", nombre_archivo)
                        archivos_con_error += 1
                else:
                    archivos_no_encontrados += 1

            cursor.execute("DELETE FROM versiones_tesinas WHERE tesina_id = ?", (tesina_id,))
            versiones_eliminadas = cursor.rowcount

            cursor.execute("DELETE FROM tesinas WHERE id = ?", (tesina_id,))
